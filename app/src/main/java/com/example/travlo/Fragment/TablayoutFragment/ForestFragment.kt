...
class ForestFragment : Fragment() {
    ...
    lateinit var rv_list_forest: RecyclerView
    ...
    override fun onCreateView(
        ...
        val view = LayoutInflater.from(parentFragment?.context)
            .inflate(R.layout.fragment_forest, container, false)

        rv_list_forest = view.findViewById(R.id.rv_list_forest)
        ...
        getListDestinationForest(view)

        return view
    }

    private fun getListDestinationForest(view: View?) {
        ApiConfigDestionation.getService().getListForest()
            .enqueue(object : Callback<ResponseDestination> {
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        if (response.body()?.data?.isNotEmpty() == true) {
                            ...
                            val adapterForest = ListAllAdapter(responseList)
                            rv_list_forest.apply {
                                layoutManager = LinearLayoutManager(view?.context)
                                setHasFixedSize(true)
                                adapterForest.notifyDataSetChanged()
                                adapter = adapterForest
                            }
                        } else {
